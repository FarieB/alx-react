...
import { Header } from "./Header";  // Importing stateless Header
...
describe("<Header />", () => {
  ...
  it("mounts the Header component with no user. The logoutSection is not created", () => {
    const wrapper = shallow(<Header />);
    expect(wrapper.find("#logoutSection")).toHaveLength(0);
  });

  it("mounts the Header component with user data. The logoutSection is created", () => {
    const wrapper = shallow(<Header user={USER} />);
